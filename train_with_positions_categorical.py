...
add one-hot position indicator to each number
...
hidden_size = 64
weights_file = 'model_weights/model_weights_with_positions_categorical_{}_steps_{}.hdf5'.format( 
	n_steps, hidden_size )
...
# variable names a la Matlab

n = np.arange( n_steps )
k = to_categorical( n )
kk = np.tile( k, ( x.shape[0], 1 ))
kk = kk.reshape( -1, n_steps, n_steps )
